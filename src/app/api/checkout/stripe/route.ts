...
export async function POST(request: Request) {
  try {
    ...
    const sessionConfig: Stripe.Checkout.SessionCreateParams = {
      ...
      metadata: { planChoice: plan,  userId: user.id}, // Useful for tracking the overall Checkout Session object
...
export async function GET(req: NextRequest) {
  try {
    ...
    if(plan === 'free'){
      ...
      return NextResponse.redirect(new URL('/dashboard?message=Welcome to your Free workspace!', req.url))
    }
    ...
    if (!priceId) {
      return NextResponse.redirect(new URL('/dashboard?error=invalid_plan', req.url))
    }
    ...
    return NextResponse.redirect(new URL('/dashboard?error=stripe_fail', req.url))
  }
}